...
export default function LoginPage({ theme, toggleTheme }) {
    ...
    try {
      if (forgotMode) {
        ...
        const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
          redirectTo: `${APP_URL}${RESET_REDIRECT_PATH}`
        });
      ...
      if (isSignup) {
        ...
        const { error: signUpError } = await supabase.auth.signUp({
          ...
          options: { data: { name: form.name.trim(), role: "student" } }
        });
  ...
  return <div className="login-bg">
    ...
    <div className="login-card">
      ...
      <div className="security-note">🔐 Your account role is controlled by the system. Students cannot create administrator accounts.</div>
    </div>
  </div>;
}
