...
SELECT
  ...
  u.email,
  CASE WHEN lower(COALESCE(u.email, '')) = 'admin@studentfeedback.in' THEN 'admin' ELSE 'student' END
...
CREATE TABLE IF NOT EXISTS feedbacks (
  ...
  semester TEXT CHECK (semester IN ('Semester 1','Semester 2')),
...
ALTER TABLE feedbacks ADD COLUMN IF NOT EXISTS semester TEXT;
DO $$ BEGIN
  ALTER TABLE feedbacks DROP CONSTRAINT IF EXISTS feedbacks_semester_check;
  ALTER TABLE feedbacks ADD CONSTRAINT feedbacks_semester_check
    CHECK (semester IS NULL OR semester IN ('Semester 1','Semester 2'));
END $$;
...
-- Only admins may delete a feedback after recording a resolved action.
GRANT DELETE ON TABLE public.feedbacks TO authenticated;
DROP POLICY IF EXISTS "No one can delete feedback" ON feedbacks;
DROP POLICY IF EXISTS "Admins can delete resolved feedback" ON feedbacks;
CREATE POLICY "Admins can delete resolved feedback" ON feedbacks FOR DELETE
USING (
  public.is_admin()
  AND EXISTS (
    SELECT 1 FROM public.actions
    WHERE actions.feedback_id = feedbacks.id
      AND actions.status = 'Resolved'
  )
);
