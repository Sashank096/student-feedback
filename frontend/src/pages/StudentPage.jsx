...
export default function StudentPage({ session, theme, toggleTheme }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const currentProfile = await ensureStudentProfile();
      ...
      const { error: dbError } = await supabase.from("feedbacks").insert({
        ...
        student_name: currentProfile?.name || "Student",
        text: clean,
    ...
    <main className="student-container">
      ...
      {view === "write" ? <section className="card feedback-compose">
        ...
        <form onSubmit={handleSubmit} className="feedback-form">
          ...
          <label className="action-label">Academic semester
            <select className="action-select" value={semester} onChange={e => setSemester(e.target.value)}>
